    ...
    (select 
      ...
      (select *, extract(epoch from gateway_time) gateway_seconds, ((row_number() over ()) + 1) rownum from
        (select pl.circuit_id, c.meter meter_id, l.date meter_time, pl.created gateway_time from 
          ...
          where l.id=pl.id and 
                pl.circuit_id=c.id
          order by meter_id, gateway_time) ordered_log 
      ) first_pairs,
...
"""
  Same as above, with a union of a record for each meter at the current time
  so that we can do "currently down" assessment
"""
gap_query_union = """
select * from 
  (select 
